<template>
    <div class="oa-login" v-bgInmage="bg">
        
        <div class="logo" :class="{active: showSign}">
            <h3 class="title">迪拜OA系统 </h3>
            <span class="begain" @click="experience">立即使用</span>
        </div>
            ...
            <el-form ref="signForm" label-width="80px" class="signForm" :rules="signRules" :model="signForm">
                ...
                <el-form-item prop="regcode" class="regcode-box">
                    <el-input v-model="signForm.regcode" placeholder="验证码" @keyup.enter.native="enter(islogin)">
                        <i class="iconfont icon-yingyong" slot="prepend"></i>
                    </el-input>
                    <canvas ref="regcode" width="90" height="38"></canvas>
                </el-form-item>

            </el-form>
            <button @click="enter(islogin)" style="background-color:#999">
...
<script>
    ...
    import bg from '../assets/img/oa_office.png';
    export default {
        name: 'login',
        data() {
            ...
            signUp() {
                ...
                api.signUp(params).then(r => {
                    if (r.code === 0) {
                        ...
                        this.$notify({
                            title: '注册成功',
                            message: `你的迪拜号为：${r.data}，你可以凭此登录，祝您生活愉快！`,
                            duration: 5000,
                            type: 'success'
                        });
                        this.islogin = true;
