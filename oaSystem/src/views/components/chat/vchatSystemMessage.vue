<template>
    <div class="vchat-system-Message" v-fontColor="user.chatColor">
        <ul v-if="InfoList.length">
            <template v-for="v in InfoList">
                <li v-if="v.type === 'validate'" :key="v['_id']">
                    <div class="vchat-line1 info" style="width:150px">{{v.state === 'friend' ? v.nickname + '加您好友' :v.nickname + '申请加入' + v.groupName}}</div>
                   
                     <button style="width:29px;color:#000" @click="refuse(v)">拒绝</button>
                                <button style="width:29px;color:#000"  @click="agree(v)">同意</button>
                    <el-popover
                            placement="left"
                            width="400"
                            v-model="v.visible">
        ...
        <p style="text-align:left" v-else>暂无系统消息！</p>
    </div>
</template>
